...
volatile bool taskStarted = false;
...
void createThread(void* param) {
	taskStarted = true; 
	std::printf("test thread created \n");
	while (1) {
        std::printf("createThread running \n");
        vTaskDelay(1000);
  }
}
...
void TEST_createThread(void) {
		...
		CU_ASSERT_EQUAL(taskStarted, true);
}
...
void threadCUnit(void* param) {
    ...
    CU_add_test(myTestSuite, "my test", TEST_myPow);
		CU_add_test(myTestSuite, "createThread test", TEST_createThread);
    // set verbose mode
